...
def capture_video(target_path, servo_queue, camera_id=0):
    ...
    while cv2.waitKey(1) != ord("q"):
        ...
        for face in detected_faces:
            recognized_face = recognizer.match(
                target, target_face[0][:-1], frame, face[:-1]
            )
        ...
        tm.stop()
        frame = visualize(
            frame, detected_faces, matches=matches, scores=scores, fps=tm.getFPS()
        )
...
def main():
    ...
    if SERVO_PORT is None:
        print("错误: 未检测到 CH340 串口设备，程序退出。")
        return

    if CAMERA_ID is None:
        print("错误: 未检测到可用摄像头，程序退出。")
        return
    ...
    servo_thread = threading.Thread(
        target=servo_control, args=(servo_manager, servo_queue), daemon=True
    )
    servo_thread.start()
    print("舵机控制线程已启动")

    capture_video(
        target_path="./images/target3.jpg", servo_queue=servo_queue, camera_id=CAMERA_ID
    )
