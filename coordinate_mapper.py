...
class CoordinateMapper:
    def __init__(
        self, calibration_file, camera_int_file, camera_ext_file, obj_height=5
    ):
        # 标定板数据和相机参数路径
    ...
    def _load_calibration(self):
        ...
        with open(self.calibration_file, "r") as file:
            ...
            for point in data["calibration_points"]:
                ...
                self.pixel_coords.append(
                    [point["pixel_coordinates"]["u"], point["pixel_coordinates"]["v"]]
                )
                self.workspace_coords.append(
                    [
                        point["workspace_coordinates"]["x"],
                        point["workspace_coordinates"]["y"],
                    ]
                )
        # 相机内参
    ...
    def update_affine_matrix(self):
        ...
        self.affine2d_matrix = cv2.estimateAffine2D(
            ws_subpoints, np.array(self.workspace_coords)
        )[0]
    ...
    def select_point(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if workspace_coord is not None:
                print(
                    f"Selected pixel: ({x}, {y}) -> Workspace: ({workspace_coord[0]:.2f}, {workspace_coord[1]:.2f})"
                )
            else:
